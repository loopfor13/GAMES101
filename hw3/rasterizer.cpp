...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for(x=(int)std::floor(min_x); x<=(int)std::ceil(max_x); x++){
        for(y=(int)std::floor(min_y); y<=(int)std::ceil(max_y); y++){
            ...
            if(inside){
                ...
                Eigen::Vector3f interpolated_color = alpha * color[0] / v[0].w() + beta * color[1] / v[1].w() + gamma * color[2] / v[2].w();
                interpolated_color *= w_reciprocal;

                Eigen::Vector3f interpolated_normal = alpha * normal[0] / v[0].w() + beta * normal[1] / v[1].w() + gamma * normal[2] / v[2].w();
                interpolated_normal *= w_reciprocal;

                Eigen::Vector2f interpolated_texcoords = alpha * tex_coords[0] / v[0].w() + beta * tex_coords[1] / v[1].w() + gamma * tex_coords[2] / v[2].w();
                ...
                Eigen::Vector3f vec;
                vec << x+0.5,y+0.5,1.0f;
                auto index = get_index(x,y);
                if(z_interpolated < depth_buf[index]){
                    depth_buf[index] = z_interpolated;
                    Eigen::Vector2i pixel_pos(x, y);
                    set_pixel(pixel_pos, pixel_color);
                }
            }
        }
    }
